...
SELECT cupcakes.name, SUM(num_cupcakes) FROM cupcakes LEFT JOIN orders ON (cupcakes.id=orders.cupcake_id) GROUP BY cupcakes.name ORDER BY cupcakes.name;
...
SELECT email FROM customers WHERE id IN (SELECT customer_id FROM orders GROUP BY customer_id HAVING SUM(num_cupcakes) > 100) ORDER BY email DESC;
...
SELECT fname, lname, email FROM customers WHERE id IN (SELECT customer_id FROM orders JOIN cupcakes ON (cupcakes.id=orders.cupcake_id) WHERE name = 'funfetti' AND processed = true); 